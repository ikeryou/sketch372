...
export class Visual extends Canvas {
  ...
  private _boxNum: number = Func.instance.val(25, 50);
  ...
  protected _update(): void {
    ...
    const it = w * Func.instance.val(0.03, 0.015);

    this._box.forEach((val, i) => {
      val.setBoxSize(it, h * 0.1, it * 2);
      val.position.x = i * it - (it * this._boxNum * 0.5) + it * 0.5;
      val.position.y = -val.allSize.y * 0.25 + i * (val.allSize.y * 0.5 / this._boxNum) * 0
    });
